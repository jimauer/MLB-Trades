...
FanGMaster %>% right_join(mtradeaway,by=(c("playerid"="key_fangraphs"))) %>% 
...
FanGMaster %>% right_join(mtradefor,by=(c("playerid"="key_fangraphs"))) %>% 
  filter(Season>=season,Team=="SEA") %>% group_by(playerid,Name,GM) %>% 
  summarise(WAR=sum(WAR),season=min(season)) %>% 
  ungroup() %>% 
  arrange(desc(WAR)) %>% 
  ...
  gtsave("/Users/jimauer/Documents/GitHub/MLB-Trades/Most WAR obtined by Ms.png")

#linking to Fangraphs, summarising WAR post trade and making a GT table of 
#players with least WAR since coming to Ms
fangraphs %>% right_join(mtradefor,by=(c("playerid"="key_fangraphs"))) %>% 
  filter(Season>=season,Team=="SEA") %>% group_by(playerid,Name,GM) %>% 
  summarise(WAR=sum(WAR),season=min(season)) %>% 
  ungroup() %>% 
  arrange(WAR) %>% 
